...
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_droid_wan22_amd_trust_region_lora"}
...
ENABLE_TOKEN_WEIGHTING=${ENABLE_TOKEN_WEIGHTING:-true}
MOTION_PRIOR_STRENGTH=${MOTION_PRIOR_STRENGTH:-4.0}
MOTION_PRIOR_LEARNABLE=${MOTION_PRIOR_LEARNABLE:-true}
INITIAL_WEIGHT_TARGET=${INITIAL_WEIGHT_TARGET:-0.7}
WEIGHT_MEAN_TARGET=${WEIGHT_MEAN_TARGET:-0.5}
WEIGHT_MEAN_REG_COEFF=${WEIGHT_MEAN_REG_COEFF:-0.05}
WEIGHT_ENTROPY_COEFF=${WEIGHT_ENTROPY_COEFF:-0.0}
USE_TEACHER_HIDDEN_FOR_W=${USE_TEACHER_HIDDEN_FOR_W:-false}
...
"${RUN_CMD[@]}" \
    ...
    num_views=3 \
    model=dreamzero/vla \
    model/dreamzero/action_head=wan_amd_distill_trust_region \
    ...
    action_head_cfg.config.enable_token_weighting=$ENABLE_TOKEN_WEIGHTING \
    action_head_cfg.config.motion_prior_strength=$MOTION_PRIOR_STRENGTH \
    action_head_cfg.config.motion_prior_learnable=$MOTION_PRIOR_LEARNABLE \
    action_head_cfg.config.initial_weight_target=$INITIAL_WEIGHT_TARGET \
    action_head_cfg.config.weight_mean_target=$WEIGHT_MEAN_TARGET \
    action_head_cfg.config.weight_mean_reg_coeff=$WEIGHT_MEAN_REG_COEFF \
    action_head_cfg.config.weight_entropy_coeff=$WEIGHT_ENTROPY_COEFF \
    action_head_cfg.config.weight_predictor_use_teacher_hidden=$USE_TEACHER_HIDDEN_FOR_W
